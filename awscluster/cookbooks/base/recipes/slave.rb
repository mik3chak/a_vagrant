...
execute "add_localhost_to_known_hosts" do
	...
	#notifies :run, "execute[add_other]", :immediately
	action :nothing
end

#execute "add_other" do
#	user node['user']['hdfs']
#        command "ssh-keyscan ip-10-0-0-203 >> /home/#{node['user']['hdfs']}/.ssh/known_hosts"
#        action :nothing
#end
...
while num <= node['endnodeno'].to_i
	execute "known_hosts_node" do
		user node['user']['hdfs']
		command "ssh-keyscan ip-10-0-0-#{200 + num} >> /home/#{node['user']['hdfs']}/.ssh/known_hosts"
