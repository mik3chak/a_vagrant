...
if node['hostname'] != "node#{200 + num}"
	script "add_other_nodes_to_known_hosts" do
		interpreter "bash"
        	code <<-EOH
                cat /home/#{node['user']['hdfs']}/.ssh/id_rsa.pub >> /home/#{node['user']['hdfs']}/.ssh/known_hosts

                sed -i 's/ssh-rsa/node#{200 + num} ssh-rsa/' /home/#{node['user']['hdfs']}/.ssh/known_hosts
        	EOH
	end
end
num += 1
